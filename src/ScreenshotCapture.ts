...
export class ScreenshotCapture {
  ...
  constructor(options: ScreenshotCaptureOptions = {}) {
    ...
    this.restrictionElement.setAttribute(
      'style',
      `position: fixed; top: 0; left: 0; z-index: -9999; width: ${this.chunkWidth}px; height: ${this.chunkHeight}px; overflow: clip; isolation: isolate; pointer-events: none;`,
    );
  ...
  protected async requestCaption(): Promise<{
    ...
    const restrictionTarget = await window.RestrictionTarget.fromElement(
  ...
  protected async buildScreenshot(
    item: ScreenshotCaptureTask,
    imageCapture: ImageCapture,
    ...
    if (!('scheduler' in window)) {
      throw new Error(
        'Prioritized Task Scheduling API is not supported in current browser',
      );
    }
  ...
  async *getScreenshots(
    taskList: ScreenshotCaptureTask[],
    ...
    try {
      ...
      for (const taskItem of taskList) {
        const offscreenCanvas = await this.buildScreenshot(
          taskItem,
          requestCaption.imageCapture,
        );
        yield offscreenCanvas.convertToBlob({
